...
flysasLibrary.main = flysasLibrary.main || (function(window, document) {
  ...
  function toggleAccordion(elem, event) {
    ...
    if (accordionContainer.classList.contains("expanded")) {
      ...
      if (!event) {
        setTimeout(function() { itemsContainer.style.maxHeight = getItemsHeight(itemsContainer) + "px" }, 500);
      }
    } else {
  ...
  function bindEvents() {
    ...
    for (var i = 0; i < accordionButtons.length; i++) {
      ...
      accordionButtons[i].addEventListener("click", function(event) { toggleAccordion(this, event) });
    }
