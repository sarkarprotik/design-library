...
flysasApp.forms.sfb.widget = flysasApp.forms.sfb.widget || (function(window, document, market) {
  ...
  function fillTestData(event) {
    ...
    var organisationnumbers = {"se": "123456-7890", "no": "123456789", "dk": "12345678", "fi": "1234567-8", "lu": "12345678"};
  ...
  function sendForm(event) {
    ...
    if (!isValidForm) {
      ...
      nonValidElem.scrollIntoView({ behavior: "smooth" });
      return;
    }
    ...
    const isNordic = (code === "se" || code === "no" || code === "dk" || code === "fi")
    if (!isNordic){
    ...
    loadMarkup({ url: postDataEndpoint, data: postData }, function(response) {
